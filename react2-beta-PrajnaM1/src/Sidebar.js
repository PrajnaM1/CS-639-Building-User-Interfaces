...
class Sidebar extends React.Component {
  ...
  getInterestAreaOptions(){
    ...
    let interest_array = [];
    let keyword = 'All';
    interestAreaOptions.push(<option key={keyword}>{keyword}</option>);

    for (const course of Object.values(this.props.courses)){
      for (const keyword of course.keywords){
        //console.log("KEYWORD:", keyword);
        if (interest_array.indexOf(keyword) === -1){
          interest_array.push(keyword);
          interestAreaOptions.push(<option key={keyword}>{keyword}</option>);
        }
      }
    }
    //console.log("INTEREST:", interestAreaOptions);
    ...
    return (
      <>
        <Card style={{width: 'calc(20vw - 5px)', marginLeft: '5px', height: 'calc(100vh - 52px)', position: 'fixed'}}>
          <Card.Body>
            <Card.Title>Search and Filter <SearchIcon /></Card.Title>
            <Form>
